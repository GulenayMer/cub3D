...
/*   Updated: 2022/10/19 14:22:14 by mgulenay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cub3d_init_direction(t_data *data)
{
	...
	while (coord->y < data->map->count_y)
	{
		...
		while (coord->x < data->map->count_x)
		{
			cell = (t_cell *)matrix_get(data, coord->x, coord->y);
			if (cub3d_direction_set(data, cell, coord) == EXIT_SUCCESS)
				return ;
			coord->x += 1;
		}
		coord->y += 1;
...
int	cub3d_direction_set(t_data *data, t_cell *cell, t_coord *coord)
{
	if (cell->type == TYPE_N)
	{
...
void	*cub3d_raycast(void *input)
{
	...
	while (x < WIDTH)
	{
		...
		data->ray->delta->x = abs(1 / data->ray->pos->x);
		data->ray->delta->y = abs(1 / data->ray->pos->y);
		// Second Part
		if (data->ray->pos->x < 0)
		{
			data->ray->step->x = -1;
			data->ray->side->x = (data->ray->pos->x - data->ray->map->x) * data->ray->delta->x;
		}
		else
		{
			data->ray->step->x = 1;
			data->ray->side->x = (data->ray->map->x + 1.0 - data->ray->pos->x) * data->ray->delta->x;
		}
		if (data->ray->pos->y < 0)
		{
			data->ray->step->y = -1;
			data->ray->side->y = (data->ray->pos->y - data->ray->map->y) * data->ray->delta->y;
		}
		else
		{
			data->ray->step->y = 1;
			data->ray->side->y = (data->ray->map->y + 1.0 - data->ray->pos->y) * data->ray->delta->y;
		}
		// Third Part: DDA
		while (data->ray->hit == 0)
		{
        	...
        	{
				data->ray->side->x += data->ray->delta->y;
		...
		// Fourth Part
		if(data->ray->side == 0) 
			data->ray->wall_dist = (data->ray->side->x - data->ray->delta->x);
      	else
			data->ray->wall_dist = (data->ray->side->y - data->ray->delta->y);
		// Fifth Part
